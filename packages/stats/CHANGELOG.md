...
- Added a `model` dimension to `health_signals`: every counter row now carries the bare model id it attributes to (`""` when unattributable) and rows are keyed `(session_file, signal, model)`, so cross-session baselines can segment each signal per model. Tool-derived counters (validation failures, edit rejections, repeat reads, intent fill) attribute to the model of the assistant turn that issued the tool call; `provider_error_turns` to the erroring turn's model; stage aggregates to the `model` recorded on each `stage_timings` row (pre-model rows fold under `""`). `readHealthSignals(sessionFile, signal?)` returns the model and optionally narrows to one signal. Databases with the pre-model table shape are migrated by dropping and recreating `health_signals` — the table is a pure derivation of session JSONL — while `file_offsets` is left untouched, so rows repopulate lazily as sessions receive new content and get re-parsed rather than forcing a full re-ingest.
