...
export class AgentSession {
	...
	#generatedFileGuardBackend(): Backend | undefined {
		return (
			getBackendFromTool(this.#toolRegistry.get("edit")) ??
			getBackendFromTool(this.#toolRegistry.get("write")) ??
			getBackendFromTool(this.#toolRegistry.get("vim"))
		);
	}
	...
	#abortStreamingEditForAutoGeneratedPath(toolCall: ToolCall, path: string, resolvedPath: string): void {
		...
		const backend = this.#generatedFileGuardBackend();
		if (!backend) return;
		void assertEditableFile(backend, resolvedPath, path).catch(err => {
			// Backend reads can reject with ENOENT, etc. Only ToolError means
	...
	#preCacheStreamingEditFile(event: AgentEvent): void {
		...
		if (backend?.kind === "remote") {
			void this.#ensureFileCacheViaBackend(backend, resolvedPath);
			return;
		}
	...
	async #ensureFileCacheViaBackend(backend: Backend, resolvedPath: string): Promise<void> {
		if (this.#streamingEditFileCache.has(resolvedPath)) return;
		try {
			const read = await backend.fs.readLines(resolvedPath);
			this.#streamingEditFileCache.set(resolvedPath, normalizeToLF(read.lines.join("\n")));
		} catch {
			// Don't cache on read errors (including ENOENT) - let the edit tool handle them
		}
	}
	...
	async #checkRemovedLinesAsync(
		...
		try {
			...
			if (backend?.kind === "remote") {
				...
				text = stripBom(await Bun.file(resolvedPath).text()).text;
			}
