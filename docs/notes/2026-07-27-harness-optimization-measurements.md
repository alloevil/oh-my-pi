...
## 外部调研与复核记录（2026-07-28）

### 调研对象
1. **ECC**（github.com/affaan-m/ECC，实时拉取）：harness 之上的内容包+流程包（281 skills / 67 agents / hooks / AgentShield 配置面安全扫描），不是 harness 本体。可借鉴：配置面安全审计（我们的空白）、instincts 持续学习流程化。反面参照：281 skills 的路由面——我们 26 skill 即实测近邻混淆，mega 包批发此问题。
2. **AHE 论文**：arXiv 2604.25850（2026-04-28，复旦 NLP，已修订 v4）。观测驱动的 harness 闭环进化：组件/经验/决策三支柱。实验：10 轮进化 TB2 pass@1 69.7%→77.0%，超人工 Codex-CLI harness（71.9%）；冻结迁移 SWE-bench-verified 省 12% token。**消融关键发现：收益定位在 tools/middleware/memory，非 system prompt——独立印证本仓「放弃 prompt 手术、投入工具与中间件层」的路线。**
3. 附带发现：arXiv 2607.14108（2026-05-07）「边际工具效用」——精简工具集的量化度量，可服务 xdevTopLevelDevices 置顶决策。

### 复核与撤回（方法论记录）
- ~~「业界公认缺失实时 verdict 层」~~：归属来源（morphllm）原文无法溯源，撤回引用。主题（决策级观测/行为基线）多源讨论但均为厂商内容。
- ~~「70-95% AI agent 真实环境失败」~~：溯源至 MIT NANDA《GenAI Divide》（2025.07-10），原意为「95% GenAI 试点无可测 P&L 影响」，且方法论被公开批评（仅统计公开公告；G2 反向数据：生产 agent 失败率 <2%）。判定为被误用的统计，不采信。
- 教训重申：三手综述给方向，一手来源给依据；引用前先溯源。

### 定位结论（复核后存活）
harness 三层模型：运行时基座（商品化中）→ 可观测层（本仓已建，差异化所在）→ 进化层（AHE，前沿；本仓的金丝雀+环节统计+错例校准恰为其基座）。内容包为商品，不自建。
